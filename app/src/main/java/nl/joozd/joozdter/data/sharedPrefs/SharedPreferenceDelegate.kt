...
class SharedPreferenceDelegate<T : Any>(
...
) : ReadOnlyProperty<JoozdPreferences, SharedPreferenceDelegate.ReadOnlyPref<T>> {
    ...
    class PrefImpl<T: Any>(thisRef: JoozdPreferences, key: String, private val defaultValue: T): Pref<T>{
    ...
    override fun getValue(thisRef: JoozdPreferences, property: KProperty<*>): Pref<T> = getInstance(thisRef)

    private fun getInstance(thisRef: JoozdPreferences) = _instance ?: PrefImpl(thisRef, key, defaultValue)
}